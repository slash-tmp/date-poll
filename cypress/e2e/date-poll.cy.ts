describe('Poll creation page /poll/new', () => {
  ...
  it('creates a new poll and redirect to the admin page', () => {
    ...
    cy.getByLabel('Description (optionnel)').type('La description de mon sondage')
  ...
  it('saves data between steps', () => {
    ...
    cy.getByLabel('Description (optionnel)').should('have.value', 'La description de mon sondage')
    cy.contains('Suivant').click()
    ...
    cy.getByLabel('Recevoir un email pour chaque participation').should('have.value', 'on')
